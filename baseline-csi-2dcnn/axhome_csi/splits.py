...
def human_samples(samples: Iterable[SampleRecord]) -> tuple[SampleRecord, ...]:
    return tuple(
        sample
...
def make_subject_fold(
    samples: Sequence[SampleRecord], *, test_person: str, val_person: str
) -> SubjectFold:
...
def rotating_loso_pairs(samples: Sequence[SampleRecord]) -> list[tuple[str, str]]:
...
def validate_in_domain_split(
    split: InDomainSplit,
    *,
    expected_samples: Sequence[SampleRecord] | None = None,
) -> None:
    ...
    id_sets = [set(ids) for ids in partition_ids]
    if any(
        left & right
    ...
    assigned = tuple(
        sample for partition in partitions for sample in partition
    )
    if any(
    ...
    for name, partition in zip(
        ...
        present = {
            (sample.person_id, sample.action_id) for sample in partition
        }
        ...
        if missing:
            ...
            raise ValueError(
                f"in-domain {name} split is missing strata: {rendered}"
            )


def make_in_domain_split(
    samples: Sequence[SampleRecord], *, seed: int
) -> InDomainSplit:
    eligible = human_samples(samples)
    if not eligible:
    ...
    for sample in eligible:
        groups.setdefault((sample.person_id, sample.action_id), []).append(
            sample
        )
